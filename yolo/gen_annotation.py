...
def generate_annotation(model, path, conf_thres, nms_thres, img_size, batch_size,save_path,ifsave = True,):
    model.eval()
    ...
    for batch_i, (img_path, imgs, targets) in enumerate(tqdm.tqdm(dataloader, desc="Detecting objects")):
        ...
        if isinstance(targets,torch.Tensor):
            # 当targets为空的时候，其类型为list。不过其实好像后面也没有用到这里的targets啊……
            # Extract labels
        ...
        for i,(output,path) in enumerate(zip(outputs,img_path)):
            ...
            if output is not None:
                total_dict['seriesuid'] = id_name
                total_dict['coordX'] = (output[:,0] + output[:,2])/2
                total_dict['coordY'] = (output[:,1] + output[:,3])/2
                total_dict['coordZ'] = z
                total_dict['class'] = output[:,-1]  # 1--> nodule 2-->strip 3--> 4-->（比原先+1，对比不同）
                total_dict['probability'] = output[:,4]
    ...
    for index, row in patch_data.iterrows():
        patch_x, patch_y, patch_z, patch_id = row.coordX, row.coordY, row.coordZ, int(row.seriesuid)
        ...
        for ori_index, ori_row in ann.iterrows():
            ...
            if ((patch_x-x)/w)**2 + ((patch_y-y)/h)**2 + ((patch_z-z)/d)**2 < 1:
                patch_data.loc[index,'label'] = label_dict[ori_row.label]
    if ifsave:
        patch_data['class'] = patch_data['class'].map(class_dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weights_path", type=str, default="checkpoints/09-12-04_ap_0.37_35.pth", help="path to weights file")
    ...
    generate_annotation(
        ...
        batch_size=8,
        save_path = '../dataset/multi_valid_patch_annotation.csv'
    )
    ...
    generate_annotation(
        ...
        img_size=opt.img_size,
        batch_size=8,
        save_path = '../dataset/multi_train_patch_annotation.csv'
    )
    '''
    generate_annotation(
        model,
        ...
        img_size=opt.img_size,
        batch_size=8,
        save_path = '../dataset/multi_predict_patch_annotation.csv'
    )
