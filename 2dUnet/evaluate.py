...
def evalue_score(predict_image_path, predict_label_path):
    ...
    for image_name in image_names:
        ...
        if label in label_names:
            ...
            predict_image = cv2.imread(ssi, cv2.IMREAD_GRAYSCALE)
            label_src = cv2.imread(ssl,cv2.IMREAD_GRAYSCALE)
            if predict_image.shape[0] != label_src.shape[0]:
                ...
                predict_image = cv2.resize(predict_image, (label_src.shape[0], label_src.shape[1]))
...
if __name__ == '__main__':
    valid_label = "trainset_2d/valid/label/"
    valid_image = "trainset_2d/valid/2019-08-12_06-00_cross_loss=0.00.h5/"
